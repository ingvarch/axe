...
impl EditorBuffer {
    ...
    pub fn move_left(&mut self) {
        if self.cursor().col > 0 {
            ...
            self.cursor_mut().row -= 1;
            self.cursor_mut().col = self.line_length(self.cursor().row);
        }
        self.cursor_mut().desired_col = self.cursor().col;
    }
    ...
    pub fn move_down(&mut self) {
        if self.cursor().row + 1 < self.content_line_count() {
            ...
            self.cursor_mut().row += 1;
            let line_len = self.line_length(self.cursor().row);
            self.cursor_mut().col = self.cursor().desired_col.min(line_len);
        }
    }
    ...
    pub fn move_file_end(&mut self) {
        ...
        self.cursor_mut().row = last;
        self.cursor_mut().col = self.line_length(last);
        self.cursor_mut().desired_col = self.cursor().col;
    }
    ...
    pub fn move_page_down(&mut self, viewport_height: usize) {
        ...
        self.cursor_mut().row = (self.cursor().row + viewport_height).min(max_row);
        let line_len = self.line_length(self.cursor().row);
        self.cursor_mut().col = self.cursor().desired_col.min(line_len);
    }
    ...
    pub fn move_page_up(&mut self, viewport_height: usize) {
        self.cursor_mut().row = self.cursor().row.saturating_sub(viewport_height);
        let line_len = self.line_length(self.cursor().row);
        self.cursor_mut().col = self.cursor().desired_col.min(line_len);
    }
